...
# Uses sage for Coppersmith small-roots on a linear polynomial.
...
def recover_prime_from_fault(n, m, s_hi):
    A = (s_hi << 128) - m
...
def main():
    ...
    # Pick a large random plaintext so CRT branches differ (mp != mq).
    m = secrets.randbelow(n - 1) + 1
    c = pow(m, e, n)

    # Corrupt one block inside u (CRT coefficient) only.
    # For this format, late blocks (e.g. 39) are safely inside u across key-size variations.
    ...
    if "SID" not in out:
        raise RuntimeError(f"Fault query failed: {out}")

    s_hi = bytes_to_long(bytes.fromhex(out["SID"]))

    p = recover_prime_from_fault(n, m, s_hi)
    if p is None:
        raise RuntimeError("Failed to recover prime; retry script for a fresh instance.")
    q = n // p

    # Try both concatenation orders for the key derivation.
    for a, b in [(p, q), (q, p)]:
    ...
    raise RuntimeError("Prime recovered but flag decryption failed.")
